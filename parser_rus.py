...
WHITELISTED_SNI = old_sni_list + new_sni_list
...
# ===================== ГЛАВНАЯ ЛОГИКА =====================
def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 Сбор конфигов для 🌴ЯлтаВПН (только Россия)")
    all_configs = []
...
#profile-title: 🌴ЯлтаВПН - Курортный ВПН (только РФ конфиги)
#hide-settings: 1
{content}"""

    b64 = base64.b64encode(full_text.encode("utf-8")).decode("utf-8")
    filename = "YaltaVPN_RU.txt"
    with open(filename, "w", encoding="utf-8") as f:
        f.write(b64)

    print("\n" + "="*50)
    print("🌴ЯлтаВПН - Курортный ВПН | ТОЛЬКО РОССИЯ")
    print(f"🌴ЯлтаВПН | CIDR: {total_hosts} | SNI: {total_sni} |")
    print("📢 ТГ канал: https://t.me/YaltaVPN_Obxod")
    print("="*50)
    print(f"✅ Сохранено {len(final_lines)} российских конфигов в файл {filename} (base64)")
